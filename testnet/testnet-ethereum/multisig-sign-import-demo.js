...
// var mnemonicFromCosignUI = "below school daring add maid churn include concert spring modify frame middle salmon front amazing favorite agree elegant glance express truth test boy broccoli"
...
// let safeAddress = "0xea1e2d410dA070e7CB965E9E388838EcAd056b40";
let safeAddress = "0x826feae0c9D19Bea30CAE9E60E45B8C869F6aE3C";
let fileName = '/1697018602-ethereum-mainnet-signed-tx';
...
async function main() {
    ...
    // let privateKey3 = genDerivationPrivateKey(mnemonicFromCosignUI, rootPath, "0/0")
    ...
    let safeTransactionDataValue = safeTransaction['data'];
    ...
    partiallySignedTxData = bufferFile(fileName);
    console.log('partiallySignedTxData ', partiallySignedTxData)
    partiallySignedTxData = JSON.parse(partiallySignedTxData);
    console.log('partiallySignedTxData ', partiallySignedTxData)

    partiallySignedTx = JSON.parse(partiallySignedTxData["signedTx"])
    console.log('partiallySignedTx ', partiallySignedTx)

    partiallySignedTx["data"] = safeTransactionDataValue;
    console.log('partiallySignedTx ', partiallySignedTx)

    console.log("---------------")
    console.log("co signing")
    console.log("---------------")
    // if (balance1 > 0 || balance2 > 0) {
    let fullySignedTx = await genMultisigSignedTx(signer1, partiallySignedTx, safeAddress);
    ...
    // await exportTx(JSON.stringify(signedTxArray[signedTxArray.length - 1]))
    ...
    let tx = await broadcastSignedTx(signer1, output, safeAddress);
    console.log('tx ', tx)
}
...
async function exportTx(unsignTx) {
    // fs.writeFileSync('/tmp/test-sync', 'Hey there!');
    console.log("unsignTx ", unsignTx)
    let blockchain = "eth";
    let filename = (Math.floor(new Date().getTime() / 1000)) + "-" + blockchain + "-" + "key";
    var stream = fs.createWriteStream(filename);
    stream.once('open', function(fd) {
        stream.write(unsignTx);
        stream.end();
    });
}
