...
let safeAddress = "0x838b00C20Fc6Bcb0728EDfbc8Caf9EA992AC2453";
...
async function main() {
    ...
    await exportTx(safeTransactionToString(signedTxArray[signedTxArray.length - 1]), safeAddress, "0/0")
...
async function exportTx(unsignTx, safeAddress, derivationPath) {
    // fs.writeFileSync('/tmp/test-sync', 'Hey there!');
    console.log("unsignTx ", unsignTx)
    let blockchain = "eth";
    let filename = (Math.floor(new Date().getTime() / 1000)) + "-" + blockchain + "-" + "unsigned-tx";
    ...
    dataParsedTx = JSON.parse(dataParsed['unsignedTx']);
    console.log("dataParsedTx", dataParsedTx);
    console.log("dataParsedTx['data']", dataParsedTx['data']);

    let dataValue = dataParsedTx['data']
    console.log("dataValue", dataValue);

    let dataValueRaw = dataValue['value']
    console.log("dataValueRaw", dataValueRaw);

    let dataValueRawParsed = JSON.parse(dataValueRaw)
    console.log("dataValueRawParsed", dataValueRawParsed);

    // dataParsedTxValue = JSON.parse(dataParsedTx['data']['value']);
    // console.log("dataParsedTxValue", dataParsedTxValue);
